...
Runtime initRuntime([Message msg]) {
  var runtime = Runtime()
...
Future<AST> visitAttAccess(Runtime runtime, AST node) async {
  ...
  if (node.binaryOpRight != null &&
    ...
    return visit(runtime, node.binaryOpLeft).then((value) async {
      // TODO: PASS VALUE OF PREVIOUS FUNCTION TO THEN CALLBACK
    ...
    if (left.type == ASTType.AST_LIST) {
      if (node.binaryOpRight.type == ASTType.AST_VARIABLE) {
        if (node.binaryOpRight.variableName == 'length') {
          ...
          return intAST;
        } else if (node.binaryOpRight.variableName == 'toString') {
          var strAST = initAST(ASTType.AST_STRING);

          strAST.stringValue = left.listChildren.toString();
    ...
    if (left.type == ASTType.AST_STRING) {
      if (node.binaryOpRight.type == ASTType.AST_VARIABLE) {
        ...
        return visitStringMethods(node, left, runtime);
      }

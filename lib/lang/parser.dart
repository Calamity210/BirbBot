...
AST parseType(Parser parser, Scope scope) {
  ...
  switch (tokenValue) {
    case 'void':
      ...
      break;
    case 'Future':
      astType.isFuture = true;
      eat(parser, TokenType.TOKEN_LESS_THAN);
      type.type = parseType(parser, scope).typeValue.type;
      eat(parser, TokenType.TOKEN_GREATER_THAN);
      break;
    case 'Stream':
      type.type = DATATYPE.DATA_TYPE_STREAM;
      break;
    case 'String':
...
AST parseTerm(Parser parser, Scope scope) {
  ...
  while (parser.curToken.type == TokenType.TOKEN_DIV ||
      ...
      parser.curToken.type == TokenType.TOKEN_MOD ||
      ...
      switch (astVarDef.variableValue.type) {
        ...
        case ASTType.AST_ENUM:
          if (astType.typeValue.type != DATATYPE.DATA_TYPE_ENUM)
            parserTypeError(parser);
          break;
        case ASTType.AST_STREAM:
          if (astType.typeValue.type == DATATYPE.DATA_TYPE_VAR) {
            astType.typeValue.type = DATATYPE.DATA_TYPE_STREAM;
            astVarDef.variableType = astType;
          }
          if (astType.typeValue.type != DATATYPE.DATA_TYPE_STREAM)
            parserTypeError(parser);
          break;

...
void initStandards(Runtime runtime, Message msg) async {
  globalMsg = msg;

  registerGlobalVariable(runtime, 'birbVer', '0.0.1');
  registerGlobalFutureFunction(runtime, 'screm', funcScrem);
  ...
  registerGlobalFutureFunction(runtime, 'input', funcInput);
  ...
  registerGlobalFutureFunction(runtime, 'delay', funcDelay);
  registerGlobalFunction(runtime, 'GETStream', funcGETStream);
  registerGlobalFutureFunction(runtime, 'listen', funcListenStream);
}
...
/// STDOUT
Future<AST> funcScrem(Runtime runtime, AST self, List args) async {
...
/// STDIN
Future<AST> funcInput(Runtime runtime, AST self, List args) async {
  var astString = initAST(ASTType.AST_STRING);
  astString.stringValue = (await globalMsg.client.onMessage.first).message.content;
...
AST funcDecodeJson(Runtime runtime, AST self, List args) {
  ...
  AST jsonAST;
  if (decoded is List)
    jsonAST = initAST(ASTType.AST_LIST)..listChildren = decoded;
  else
...
AST funcGETStream(Runtime runtime, AST self, List args) {
  runtimeExpectArgs(args, [ASTType.AST_STRING, ASTType.AST_INT]);

  AST urlAST = args[0];
  AST durationAST = args[1];
  StreamController controller = StreamController.broadcast();

  Duration duration = Duration(milliseconds: durationAST.intVal);
  Timer.periodic(duration, (timer) {
    get(urlAST.stringValue).then((response) {
      AST astObj = initAST(ASTType.AST_CLASS);
      astObj.variableType = initAST(ASTType.AST_TYPE);
      astObj.variableType.typeValue = initDataTypeAs(DATATYPE.DATA_TYPE_CLASS);

      // BODY
      AST ast = initAST(ASTType.AST_VARIABLE_DEFINITION);
      ast.variableName = 'body';
      ast.variableType = initAST(ASTType.AST_TYPE);
      ast.variableType.typeValue = initDataTypeAs(DATATYPE.DATA_TYPE_STRING);

      AST astVal = initAST(ASTType.AST_STRING);
      astVal.stringValue = response.body;
      ast.variableValue = astVal;
      ...
      controller.add(astObj);
    });
  });
  AST streamAST = initAST(ASTType.AST_STREAM)..streamController = controller;
  return streamAST;
}

Future<AST> funcListenStream(Runtime runtime, AST self, List args) async {
  runtimeExpectArgs(args, [ASTType.AST_STREAM]);

  StreamController controller = (args[0] as AST).streamController;

  AST classAST = await controller.stream.first;
  return classAST;
}

Future<AST> funcDelay(Runtime runtime, AST self, List args) async {
  runtimeExpectArgs(args, [ASTType.AST_INT]);

  AST intAST = args[0];

  await Future.delayed(Duration(milliseconds: intAST.intVal));
